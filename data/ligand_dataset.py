...
# Contrastive learning graph augmentation from MolCLR

# Use subgraph removal augmentation strategy: best performing one according to the original paper
...
class LigandDataset(MoleculeDataset):
    """
    ...
    def __init__(self, train=True, data_path=None, valid_size=0.1, random_seed=42, **kwargs):
        ...
        split = int(np.floor(self.valid_size * num_samples))
        
        if train:
            self.indices = indices[split:]  # Training indices
        else:
            self.indices = indices[:split]  # Validation indices
